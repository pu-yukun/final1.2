...

{
    ...
    public bool test;
public EnemyZombie EnemyZombie;
...
//public static event Action  onDead;
 [Header("UI 设置")]
   ...
   private bool isDisplayZhan;  
    ...
    void Start()  
    {      health = 200f; // 强制覆盖初始值
EnemyZombie=GetComponent<EnemyZombie>();
 if (zhanImage == null)  
        {  
...
void Update(){
      ...
      Debug.Log($"{gameObject.name} 测试生命值: {health}"); // 输出具体怪物名称和伤
  ...
  if (!isDead && transform.position.y < -50f)
    ...
    // 武器对应的伤害值  
    private readonly float[] weaponDamage = { 25f, 40f, 45f, 0f, 10f, 200f,1999f };  
...
EnemyZombie.ApplyFlashEffect();
  ...
  if (other.CompareTag("Bullet")||other.CompareTag("RPG"))  
        ...
        {  
            Debug.Log("我的子弹实体找不到了啊啊啊啊啊啊啊啊啊啊大大！");  
        }   
       */
     if (other.CompareTag(bugTriggerTag))
        {
  ...
  private void OnTriggerExit(Collider other)
    {
        ...
        if (other.CompareTag(bugTriggerTag))
        {
            Debug.Log($"{gameObject.name} 离开卡bug区域");
            isInBugZone = false;
        }
    }
...
/*
public void ShowZhanUI()
{
    if (zhanImage != null)
    {
        zhanImage.gameObject.SetActive(true);
      Invoke("HideZhanUIAfterSeconds",1.5f);
    }
}

public void HideZhanUIAfterSeconds()
{
   // yield return new WaitForSeconds(seconds);
    zhanImage.gameObject.SetActive(false);
}
*/
    ...
    {  if (isDead) return; 
       ...
       // UpdateHealthUI(); // 更新血量UI  
        ...
        {  
         ...
         Debug.Log($"{gameObject.name} 当前生命值: {health}，受到伤害: {damage}"); // 输出具体怪物名称和伤
    }  
/*
